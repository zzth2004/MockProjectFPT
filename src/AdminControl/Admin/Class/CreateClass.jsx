...
import { 
  ArrowLeft, Save, School, User, Calendar, 
  Video, Users, BookOpen, Clock, Loader2 
} from "lucide-react";
...
export default function CreateClass() {
  ...
  const handleSubmit = async () => {
    ...
    // Kiểm tra User Auth
    if (!user || !user.id) {
        alert("Lỗi phiên đăng nhập: Không tìm thấy ID người dùng. Vui lòng F5 hoặc đăng nhập lại.");
        return;
    }
    ...
    try {
      ...
      const msg = error.response?.data?.message || "Lỗi Server (500). Kiểm tra lại TeacherID hoặc Date.";
    ...
    <div className="min-h-screen bg-[#F8F9FC] font-sans pb-20 p-4 md:p-6 animate-in fade-in duration-500">
      ...
      <div className="flex justify-between gap-4 mb-8 sticky top-0 z-30 bg-[#F8F9FC]/90 backdrop-blur-sm py-2">
        <div className="flex items-center gap-4">
           <button onClick={() => navigate(-1)} className="w-10 h-10 rounded-xl bg-white hover:bg-gray-100 flex items-center justify-center shadow-sm"><ArrowLeft size={20} /></button>
           <div><h1 className="text-2xl font-black text-gray-900 uppercase italic">Tạo <span className="text-[#2d5a2d]">Lớp học</span></h1></div>
        </div>
        <button disabled={isLoading} onClick={handleSubmit} className="px-6 py-2.5 rounded-xl font-bold text-white bg-[#2d5a2d] hover:bg-[#1a3d1a] shadow-lg flex items-center gap-2">
           {isLoading ? <Loader2 size={18} className="animate-spin" /> : <Save size={18} />} Lưu lớp học
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 max-w-[1600px] mx-auto">
        ...
        <div className="lg:col-span-2 space-y-8">
           <div className="bg-white p-6 rounded-[2rem] border-none shadow-sm space-y-6">
              ...
              <div>
                 ...
                 <input className="w-full p-4 bg-gray-50 rounded-2xl font-bold border-none outline-none focus:ring-2 focus:ring-[#2d5a2d]/20" value={formData.name} onChange={(e) => handleChange("name", e.target.value)} placeholder="VD: Lớp Tiếng Hàn K15" />
              </div>

              <div className="grid grid-cols-2 gap-6">
                 <div>
                    <label className="text-[11px] font-black uppercase text-gray-400">Lịch học (Schedule Description)</label>
                    <div className="flex items-center bg-gray-50 p-4 rounded-2xl gap-2">
                        <Clock size={18} className="text-gray-400"/>
                        <input className="bg-transparent font-medium outline-none w-full" value={formData.schedule} onChange={(e) => handleChange("schedule", e.target.value)} placeholder="VD: Tối 2-4-6 lúc 19:00" />
                    </div>
                 </div>
                 <div>
              ...
              <div>
                 ...
                 <div className="flex items-center bg-blue-50 p-4 rounded-2xl gap-2 text-blue-700">
                    <Video size={18}/>
                    <input className="bg-transparent font-medium outline-none w-full placeholder-blue-300" value={formData.googleMeetLink} onChange={(e) => handleChange("googleMeetLink", e.target.value)} placeholder="https://..." />
                 </div>
              </div>
           </div>
        </div>
        ...
        <div className="space-y-6">
           <div className="bg-white p-6 rounded-[2rem] border-none shadow-sm space-y-6">
              ...
              <div>
                 <label className="text-[10px] font-black uppercase text-gray-400 mb-2 block">Giáo viên (Auto-Assign)</label>
                 ...
                 {user ? (
                     <div className="flex items-center gap-3 bg-white p-2 rounded-2xl border border-gray-100 shadow-sm">
                        ...
                        <div className="flex flex-col">
                            <span className="text-sm font-black text-[#1e293b]">{user.fullName || "User"}</span>
                            <span className="text-[10px] font-bold text-gray-400 italic">ID: {user.id}</span>
                        </div>
                     </div>
                 ) : (
                     <div className="p-2 bg-red-50 text-red-500 rounded text-xs">⚠ Không tìm thấy User</div>
                 )}
                 <p className="text-[9px] text-gray-400 mt-1 italic ml-1">* Hệ thống sẽ tự gán User ID này làm Teacher ID</p>
              </div>
